...
// ── apiRequest shim ── bridges legacy callers to api()
async function apiRequest(path, method = 'GET', body = null) {
  return api(method, path, body || null);
}
...
function _showCodePushNotification(item) {
  ...
  el.innerHTML = `
    ...
    <div style="display:flex;gap:8px">
      <button onclick="navigator.clipboard?.writeText('${item.code}');toast('Code copied','success')" style="flex:1;padding:8px;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.3);border-radius:8px;color:#fff;font-family:inherit;font-size:13px;font-weight:600;cursor:pointer">📋 Copy Code</button>
      <button onclick="APP.unit_id='${item.unit_id}';this.closest('#code-push-banner').remove();openWizard('${item.unit_id}')" style="flex:1;padding:8px;background:rgba(96,165,250,.25);border:1px solid rgba(96,165,250,.5);border-radius:8px;color:#fff;font-family:inherit;font-size:13px;font-weight:600;cursor:pointer">⚡ Go to Wizard</button>
    </div>
  `;
