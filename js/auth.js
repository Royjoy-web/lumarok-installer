...
async function doLogin() {
  ...
  try {
    ...
    if (typeof afterLogin === 'function') afterLogin(data);
...
// ── ACCOUNT SETUP (from email link) ──────────────────────────
// On load, check for ?token=&email= in URL
...
async function doAccountSetup() {
  ...
  if (!pass || pass.length < 8) { toast('Password must be 8+ characters', 'error'); return; }
  ...
  try {
    ...
    // ISS-011 FIX: use setToken() so token stays in memory; cookie set by server
    if (typeof setToken === 'function') setToken(data.token);
    // ISS-011 FIX: refresh token stored as httpOnly cookie by server — do not write to localStorage
    APP.user    = data.user;
    APP.unit_id = data.user.units?.[0]?.unit_id || null;
    // Clear URL params
    history.replaceState({}, '', location.pathname);
    toast('Account activated! Welcome to LumaRoK 🎉', 'success');
    goTo('installer-hub');
    await loadInstallerOverview();
  } catch(err) {
